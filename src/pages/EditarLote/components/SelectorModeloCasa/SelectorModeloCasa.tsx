...
import { Home, Square, Bath, Bed, Building2, DollarSign, X, ChevronDown } from 'lucide-react';
...
const SelectorModeloCasa: React.FC<SelectorModeloCasaProps> = ({
  ...
  const modeloActual = modelosCasaDisponibles.find(m => m.uid === modeloCasaSeleccionado);
      ...
      <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
        ...
        >
          ...
          <div className="text-left flex-1">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {modeloActual
                ? `${modeloActual.nombre} - ${formatearPrecio(modeloActual.precioBase)}`
        ...
        {modeloCasaSeleccionado && (
          <button
            ...
            className="ml-3 flex items-center gap-2 px-3 py-1.5 text-sm text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
          >
            <X size={16} />
          </button>
        )}
